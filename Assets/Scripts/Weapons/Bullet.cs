...
public class Bullet : MonoBehaviour
{
     ...
     * Setup the bullet
     */
    virtual public void Setup(bool isStatic, Vector3 direction, float speed, float damages, Transform shooterTransform, LayerMask damageableLayerMask, bool destroyAfterDamage = true)
    {
        m_isStatic = isStatic;
    ...
    protected void OnTriggerEnter(Collider collider)
    {
        ...
        if (Mathf.Pow(2, collider.gameObject.layer) == m_damageableLayerMask.value)
        {
            ...
            {
                collidedObject.Damage(m_damages, m_shooterTransform);
